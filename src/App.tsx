...
const PromoPopup = () => {
  ...
  useEffect(() => {
    const sessionSeen = sessionStorage.getItem("promoSeen_v4_70");
    if (sessionSeen) return;

    const timeout = setTimeout(() => {
      setIsVisible(true);
      sessionStorage.setItem("promoSeen_v4_70", "true");
    }, 20000); // 20 seconds
      ...
      {isVisible && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg bg-gradient-to-br from-zinc-950 to-zinc-900 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(178,31,45,0.2)] text-center p-8 md:p-12"
          >
            {/* Soft Red Glows */}
            ...
            <button 
              onClick={closePopup}
              className="absolute top-6 left-6 text-white/30 hover:text-white transition-colors p-2"
            >
              <X size={24} />
            </button>

            <div className="relative z-10 flex flex-col items-center">
              <div className="w-16 h-16 bg-primary-red/10 rounded-2xl flex items-center justify-center mb-6 text-primary-red">
                <Sparkles size={32} />
              </div>

              <h2 className="text-2xl md:text-3xl font-bold mb-4">خصم لفترة محدودة</h2>
              
              <div className="relative mb-6">
                <div className="absolute inset-0 bg-primary-red/20 blur-2xl rounded-full scale-150 animate-pulse" />
                <span className="text-7xl md:text-8xl font-black text-primary-red relative drop-shadow-2xl">
                  70%
                </span>
              </div>

              <h3 className="text-xl md:text-2xl font-bold mb-4">بـسـعر خـاص جـداً</h3>
              
              <p className="text-white/60 mb-8 leading-relaxed text-balance">
                ابدأ الآن وتعلّم صناعة الإعلانات بالذكاء الاصطناعي بسعر خاص لفترة محدودة، مع الوصول إلى كل تحديثات الكورس القادمة.
              </p>

              <div className="w-full space-y-4">
                <p className="text-sm font-bold text-primary-red uppercase tracking-widest animate-pulse">
                ...
                <a 
                  ...
                  className="w-full py-5 bg-primary-red hover:bg-red-700 text-white text-xl font-bold rounded-2xl transition-all hover:scale-[1.02] shadow-[0_10px_30px_rgba(178,31,45,0.3)] flex items-center justify-center gap-2 group relative overflow-hidden"
                >
                  <motion.div 
                  ...
                  <ArrowRight size={20} className="relative z-10 group-hover:translate-x-[-4px] transition-transform rotate-180" />
                </a>
      ...
      <section id="faq" className="py-24 px-6 bg-zinc-950">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="space-y-2">
            <FAQItem 
              question="هل توجد متابعة مباشرة معك داخل الكورس؟" 
              answer="نعم، سيكون هناك متابعة مباشرة معي، بالإضافة إلى تقييمات للأعمال والتطبيقات التي يتم تنفيذها داخل الكورس. سأساعدك على فهم الأخطاء، وتوضيح المشاكل والتحديات التي قد تؤثر على جودة النتيجة، حتى تتمكن من تحسين شغلك والوصول لنتائج أكثر احترافية" 
            />
            <FAQItem 
